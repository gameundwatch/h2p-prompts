...
入力は **`.h2p/source/refactored.html`（Phase4の整理済みHTML）**、
`.h2p/phase2-requirements.md`（意図・スコープ・契約）、
...
入力が元HTMLではなく**リファクタ後HTML**であることが重要。意図に沿って
整理された後のHTMLは、コンポーネント境界も状態の流れも明瞭で、技術判断の
証拠として質が高い（リファクタを技術選定の前に置いた狙いがここで効く）。
...
- **Router**
  一段目：図4・Phase1(d)に画面遷移が**あるか**。なければカテゴリごと不採用。
...
## 成果物の書き込み
`.h2p/phase5-stack.md` に以下を構造化して書く。
- 各カテゴリ：一段目の要否判定（採用/不採用＋証拠）、二段目の選択結果
  （選んだ候補＋ユーザーが選んだ理由）、バージョン方針。
- スタイリング戦略：選んだ方式と理由。
- `backend: mock` 時：バック言語/FW、契約方式、ディレクトリ層構成の方針。
- パッケージマネージャ・バージョン管理・テスト手法。
- **次Phaseでのスキャフォールドに使う具体コマンドの想定**（`npm create vite`
  の指定等）。Phase7/8 のエージェントはこれを読んで bash で実行する。
