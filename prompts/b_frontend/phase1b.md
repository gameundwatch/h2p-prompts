...
## 手順
1. `origin/` を読み、棚卸しを行う（原本は変更しない）。
2. 診断観点で現状の非機能要件の状態を検出する。
3. 棚卸し＋診断の結果をユーザーに提示する。特に「拡張の妨げになりうる箇所」を
   事実とリスクの形で示す（決めつけず、判断は後のPhaseに委ねる）。
   対象範囲が広い場合は、どのモジュール/領域を対象にするか（analysis_target）を
   ユーザーと確認する。
4. `.h2p/phase1-analysis.md` に構造化して書く。

## 成果物の書き込み
`.h2p/phase1-analysis.md`（ファイル名はタイプAと共通＝後半が入力タイプを
意識せず読める）に以下を書く。
- 棚卸し（依存・構成・設定・規模）。
- 診断（検出した非機能要件の状態と、拡張性へのリスク。origin の該当箇所つき）。
- 対象範囲（analysis_target）。
- 判断保留・曖昧な点。
